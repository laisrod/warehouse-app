...
describe 'Usuário cadastra um pedido' do
  ...
  it 'com sucesso' do
    ...
    expect(page).to have_content 'Sergio - sergio@email.com'
    expect(page).to have_content 'Data Prevista de Entrega: 20/12/2024'
    expect(page).not_to have_content 'Situação do Pedido: Pendente'
    expect(page).not_to have_content 'Galpão Destino: MCD - Galpão Maceio'
    expect(page).not_to have_content 'Spark Industries Brasil LTDA'
  end

  it 'e não infoma a data de entrega' do
    #Arrange
    user = User.create!(name: 'Sergio', email: 'sergio@email.com', password: '12345678')
    warehouse = Warehouse.create!(name: 'Aeroporto SP', code: 'GRU', city: 'Guarulhos', area: 100_000,
                                  address: 'Avenida do Aeroporto, 1000', cep: '15000-000',
